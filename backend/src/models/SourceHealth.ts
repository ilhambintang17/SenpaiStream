...
// Methods
SourceHealthSchema.methods.recordSuccess = async function (responseTime: number) {
    this.isUp = true;
...
SourceHealthSchema.methods.recordFailure = async function (error: string) {
    this.isUp = false;
    this.lastChecked = new Date();
...
export default mongoose.model<ISourceHealth>('SourceHealth', SourceHealthSchema);
