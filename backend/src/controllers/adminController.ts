 ...
 */
export const getSourcesHealth = async (req: Request, res: Response) => {
    try {
 ...
 */
export const getSourceConfigs = async (req: Request, res: Response) => {
    try {
 ...
 */
export const updateSourceConfig = async (req: Request, res: Response) => {
    try {
        ...
        let config = await SourceConfig.findOne({ sourceName: sourceName as 'otakudesu' | 'kuramanime' });

        if (!config) {
            // Create if not exists
            config = await SourceConfig.create({
                sourceName: sourceName as 'otakudesu' | 'kuramanime',
                baseUrl,
            ...
            config.updatedBy = (req as any).user?.username || 'admin';
            await config.save();
        }
